...
工具识别最新详细库龄快照中的 `0-180`、`181-210`、`211-240`、`241-270`、`271-300`、`301-330`、`331-365`、`366-455`、`456+` 区间，并按站点规则判断哪些区间进入附加费计费范围。

决策看板默认只展示预计附加费计费件数大于 0 的 SKU，并自动隐藏非计费 SKU 和零件数收费区间。原始报告仍保留在当前浏览器或本机私有快照中；页面提示隐藏数量，库存明细、当前费用、执行清单及其 CSV 下载使用同一计费范围。费用金额缺失但预计计费件数大于 0 的 SKU 不会被当成免收，而是保留为待补数据。
